...
plt.imshow(tso.observation.dataset.data.data.value[:, :])
plt.show()


plt.imshow(tso.observation.dataset._wavelength)
plt.show()
plt.imshow(tso.observation.dataset.wavelength.data.value[:, :])
plt.show()
...
plt.show()
data_masked = tso.cpm.design_matrix[0][50][0]
plt.imshow(data_masked.data.value)
plt.show()
...
rebinned_wave = tso.exoplanet_spectrum.spectrum.wavelength.data.value[~mask_use][4:-4:6]
